...
if os.path.exists(os.path.join(os.path.expanduser("~"), server_dir_name, "docker-compose.yml")):
    print("\nDirectory already exists! Please retry with a different directory name.")
else:
    os.mkdir(os.path.join(os.path.expanduser("~"), server_dir_name))
    wget.download(docker_compose_url, out=os.path.join(
        os.path.expanduser("~"), server_dir_name, "docker-compose.yml"))
